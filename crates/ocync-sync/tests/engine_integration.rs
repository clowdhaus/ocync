...
//!
//! Being migrated to per-subsystem files (sync_*.rs). Remaining sections:
//!
//! - **Helpers** (line ~35) - `make_digest`, `mock_client`, `mount_*` mocks, `ManifestBuilder`
//! - **Transfer state cache** (line ~499) - Progressive population, cross-repo mount, monolithic
//! - **Shutdown integration** (line ~1071) - Graceful shutdown, drain deadline
//! - **Concurrent execution** (line ~1228) - Non-sequential ordering, parallel transfers
//! - **Nested index rejection** (line ~1409) - Index-in-index error handling
//! - **Cache invalidation** (line ~1499) - State verification after invalidation
//! - **Partial failure / concurrent dedup** (line ~1638) - Child failure, blob failure, dedup
//! - **Staging pull-once** (line ~1971) - Staging pull-once semantics, drain deadline expiry
//! - **Staging disk verification** (line ~2273) - Staged files exist on disk after sync
//! - **Warm cache skip** (line ~2397) - Warm cache skips blob entirely (no HEAD)
//! - **Batch blob checker** (line ~2481) - Batch pre-population, fallback on error
//! - **Platform filtering** (line ~3743) - Index manifest platform selection
//! - **Multi-target independence** (line ~4023) - Target isolation, partial target failure
//! - **Discovery optimization** (line ~4493) - Source HEAD cache, target staleness detection
//! - **Untriggered shutdown** (line ~6762) - Engine exits cleanly with unused signal
//! - **Concurrent mount coordination** (line ~6842) - Shared blob mount vs double-upload
//! - **Blob concurrency bound** (line ~7596) - Semaphore(6) correctness with >6 layers
//! - **Budget circuit breaker** (line ~7857) - Rate-limit pause/resume, threshold floor, refill
//! - **Artifact sync** (line ~9719) - Disabled no-op, referrer transfer, require enforcement
