...
# SSH Key Pair
#
# Generated by Terraform on apply, destroyed on `terraform destroy`. No
# operator key material lives in this repo or in Terraform state outside the
# instance lifecycle. Private key is written to a local file the xtask reads
# via bench.json.
################################################################################
...
resource "local_file" "bench_json" {
  ...
  content = jsonencode({
    ...
    user         = "ec2-user"
    ssh_key_path = abspath(local_sensitive_file.ssh_key.filename)
  })
}
