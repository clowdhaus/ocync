...
Discovery performs tag-level change detection. Every run checks both source and target — no persistent tag-level cache. This is cheap (~1-2 seconds for 500 tags × 3 targets at 50 concurrent) and eliminates the correctness risk of target-side staleness from ECR lifecycle policies, external pushes, or registry GC. No existing OCI sync tool (skopeo, regsync, crane) caches tag-level "already synced" state.

For registries where HEAD is free (Docker Hub, with `head_first: true`), discovery uses HEAD to avoid consuming rate-limit tokens. The flow:

1. HEAD source manifest → get digest from `Docker-Content-Digest` header
2. HEAD each target manifest → compare digests
3. If source == all targets → skip (zero tokens consumed)
4. If different → GET source manifest (1 token consumed), proceed with transfer

`head_first` is opt-in per registry because HEAD digest reliability varies. Docker Hub and Quay have historically had bugs where `Docker-Content-Digest` on HEAD differs from the body digest on GET during manifest schema conversion. Default is `false` (GET everything). If the `Docker-Content-Digest` header is missing or malformed on a HEAD response, fall back to GET silently — never error out.

Runtime verification: on the first HEAD response from a source registry, check whether `ratelimit-remaining` header is present. If it appears on HEAD (meaning HEADs are being counted), disable HEAD optimization for that registry and fall back to GET-only. Self-verifying rather than assumption-dependent.

Source HEAD has near-zero value when Chainguard is the source (no rate limits — GET is free anyway). This optimization primarily targets Docker Hub, GHCR, and other rate-limited third-party sources.
...
5. `head_first` is opt-in, not default. HEAD digest reliability is registry-specific. The fallback (GET everything) must be the default. Operators enable it for registries they trust.
...
7. Tag-level change detection is always live. Every run HEAD-checks both source and target manifests. No persistent tag-level cache — target-side staleness from lifecycle policies, external pushes, or GC is always detected.
...
| HEAD/GET digest mismatch (Docker Hub) | Source HEAD optimization | False skip → target has stale content | Opt-in only. Add `ocync_head_skip_total` metric. Runtime self-verification via `ratelimit-remaining` header detection. |
...
| GTID digest map (persistent tag-level skip cache) | Persistent `(source_repo, tag, target) → digest` map that skips target verification for unchanged images. Dropped because: (1) target-side staleness is a correctness bug — ECR lifecycle policies can delete images between runs, and GTID would silently fail to repair the gap; (2) no existing OCI tool (skopeo, regsync, crane) implements a persistent tag-level skip cache — all re-check the target every run; (3) with target HEAD always required for correctness, GTID's remaining value (save source GETs) overlaps with `head_first`; (4) for the primary use case (Chainguard source, no rate limits), GTID saves nothing. Live HEAD checks on both source and target cost ~1-2 seconds at scale — cheap enough to do every run. |
