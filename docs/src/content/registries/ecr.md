...
ECR has per-action rate limits (`UploadLayerPart` at 500 TPS, `InitiateLayerUpload` and `CompleteLayerUpload` at 100 TPS, `PutImage` at 10 TPS). `ocync` tracks 9 independent concurrency windows -- one per API action -- so a 429 on blob uploads does not throttle manifest reads. These per-action limits are enforced pre-emptively by ocync's token-bucket layer.
