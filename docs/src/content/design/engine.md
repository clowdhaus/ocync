...
5. **Source HEAD serves two independent purposes.** (a) Discovery cache population (unconditional): every discovery cycle issues a source HEAD to detect changes via `Docker-Content-Digest`. If the HEAD fails, times out, or returns an unreliable digest, the engine falls through to a full GET. (b) Rate-limit conservation (`head_first`, opt-in): for rate-limited source registries where HEADs are free but GETs consume tokens, `head_first: true` replaces the authoritative GET with a HEAD when the digest is already cached. The two features happen to use the same HTTP method but are independent, and the discovery HEAD has its own timeout and fallback semantics.
