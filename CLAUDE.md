...
**ECR never fulfills OCI cross-repo mount** (confirmed 2026-04-16 via `xtask probe`). 193/193 observed POSTs returned 202 across four independent triggers: 178 during the first Jupyter benchmark, and 15 via a dedicated probe with wait times of 0s / 10s / 60s between source commit and mount attempt. Wait time had no effect — ECR does not need "source commit settle" time, it simply doesn't fulfill mount. The client short-circuits via `ProviderKind::fulfills_cross_repo_mount` in `ocync-distribution`: `blob_mount` returns `MountResult::SkippedByClient` on ECR targets without a network request, saving one POST per shared blob per target. The `blob_mount_unchecked` method (used by the probe) bypasses the short-circuit for future diagnostic re-confirmation. The invariant is pinned by `tests/ecr_mount.rs` (gated behind `--features ecr-integration`). If AWS ever starts honoring mount, update `ProviderKind::fulfills_cross_repo_mount` and that test together.
